  ...
  useEffect(() => {
    Promise.all([
      ...
      if (product) {
        ...
        // Load gallery images (exclude main image)
        if (product.images?.length > 1) {
          setGalleryImages(product.images.slice(1));
        } else if (product.images?.length === 1 && !product.image) {
          // If no separate image field, first is main, rest are gallery
        }
      }
      setLoading(false);
    });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 flex flex-col gap-5">
            ...
            <div>
              ...
              {imageUrl && !uploading && (
                <p className="text-xs text-green-600 mt-1 flex items-center gap-1">✅ الصورة الأساسية محددة</p>
              )}
            </div>
